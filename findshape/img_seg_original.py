...
def getAreaOfFood(img1):
	...
	#cv2.imshow('img_bigcontour',img_bigcontour)
	#cv2.waitKey(0)
	...
	#cv2.imshow('fruit_skin',fruit_skin)
	#cv2.waitKey(0)
	...
	cv2.drawContours(mask_fruit3, fruit_contour, -3, (255,255,0), 3)
	
	#cv2.imshow('mask_fruit3',mask_fruit3)
	#cv2.waitKey(0)
	...
	largest_areas = sorted(contours, key=cv2.contourArea)
	if (len(largest_areas)==1):
		skin_contour = largest_areas[-1]
	else:
		skin_contour = largest_areas[-2]
	cv2.drawContours(mask_skin, [skin_contour], 0, (0,255,0), -1)
